...
describe("🧩 spl_token_vault_program end-to-end", () => {
  ...
  // Test that init_if_needed allows initialization when ATA is pre-created
  // This prevents DoS attacks where an attacker pre-creates the vault ATA
  it("✅ Initializes vault even when ATA is pre-created (init_if_needed)", async () => {
    // Create a NEW mint for this test (to avoid conflicts with main test)
    ...
    const [newVaultState] = anchor.web3.PublicKey.findProgramAddressSync(
      [Buffer.from("vault_state"), newMint.toBuffer()],
      program.programId
    );

    // Get the expected vault ATA address
    const newVaultTokenAccount = getAssociatedTokenAddressSync(newMint, newVaultState, true);

    // ATTACKER SCENARIO: Pre-create the ATA before the vault is initialized
    // This simulates a front-running attack
    // Use getOrCreateAssociatedTokenAccount with allowOwnerOffCurve=true for PDA owners
    await getOrCreateAssociatedTokenAccount(
      provider.connection,
      authority.payer,
      newMint,
      newVaultState,  // owner = vault PDA
      true  // allowOwnerOffCurve = true for PDAs
    );

    // Verify ATA was pre-created
    const ataInfoBefore = await provider.connection.getAccountInfo(newVaultTokenAccount);
    expect(ataInfoBefore).to.not.be.null;
    console.log("📌 ATA pre-created by attacker at:", newVaultTokenAccount.toBase58());

    // NOW: Initialize the vault - should succeed despite ATA existing (init_if_needed)
    await program.methods
      .initialize()
      .accounts({
    ...
    const vaultStateData = await program.account.vaultState.fetch(newVaultState);
    expect(vaultStateData.authority.toBase58()).to.eq(authority.publicKey.toBase58());
    expect(vaultStateData.tokenMint.toBase58()).to.eq(newMint.toBase58());
    
    console.log("✅ PASSED: Vault initialized successfully despite pre-created ATA");
    console.log("   This prevents DoS attacks via ATA front-running");
  });

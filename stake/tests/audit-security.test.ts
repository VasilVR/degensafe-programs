...
describe("🔒 Audit Security Validations", () => {
  ...
  before(async () => {
    ...
    // Create reward mint with 9 decimals (matching)
  ...
  describe(" Decimal Validation", () => {
    it("🚫 Rejects pool creation with mismatched decimals", async () => {
      // Create a mint with different decimals (6 instead of 9)
      const mismatchedRewardMint = await createMint(
        provider.connection,
        admin,
        admin.publicKey,
        null,
        6 // Different from tokenMint's 9 decimals
      );
      
      const [poolIdCounter] = derivePoolIdCounterPda(tokenMint);
      const [pool] = derivePoolPda(tokenMint, 0);
      const [rewardVault] = deriveRewardVaultPda(pool, mismatchedRewardMint);
      ...
      try {
        await program.methods
          ...
          .accounts({
            ...
            pool,
            tokenMint,
            rewardMint: mismatchedRewardMint,
            rewardVault,
        ...
        expect.fail("Should have thrown DecimalMismatch error");
      } catch (err: any) {
        expect(err.error?.errorCode?.code || err.message).to.include("DecimalMismatch");
        console.log("✅ Correctly rejected mismatched decimals");
      }
    });

    it("✅ Accepts pool creation with matching decimals", async () => {
      const [poolIdCounter] = derivePoolIdCounterPda(tokenMint);
      const [pool] = derivePoolPda(tokenMint, 0);
      ...
      console.log("✅ Pool created with matching decimals");
    });
  });
  ...
  describe(" Guard update_reward_mint", () => {
    ...
    await initializeGlobalConfig(program, admin);
      ...
      testRewardMint = await createMint(
        provider.connection,
        admin,
        admin.publicKey,
        null,
  ...
  describe(" Restrict create_pool to Program Authority", () => {
    it("🚫 Rejects pool creation from non-authority", async () => {
      ...
      // Create new mints owned by non-authority

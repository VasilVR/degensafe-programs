...
describe("💸 Stake Program - User Withdrawal", () => {
  ...
  before(async () => {
    ...
    // rewardMint = tokenMint, so rewardInfo assertions are redundant
  ...
  it("User can claim rewards without unstaking by calling withdraw_stake(0)", async () => {
    ...
    // With same-token enforcement, reward account == token account.
    // Token balance increases by the reward amount (rewards paid to same account).
    const tokenBalanceChange = Number(userTokenAfterWithdraw.amount) - Number(userTokenBeforeWithdraw.amount);
    expect(tokenBalanceChange).to.be.greaterThan(0);
    console.log(
      "✅ Token balance increased by rewards:",
      tokenBalanceChange,
    );

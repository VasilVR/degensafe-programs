...
describe("🔒 Stake Program - Pool Association Security Tests", () => {
  ...
  before(async () => {
    ...
    // Same-token enforcement: reward mint = staking token mint
    // For pool A, reward = tokenMintA; for pool B, reward = tokenMintB
    rewardMint = tokenMintA; // Used as default/Pool A reward mint
    ...
    // Create Pool B (for tokenMintB) — same-token: reward = tokenMintB
    await program.methods
      .createPool(null, new anchor.BN(rewardPercentage), poolId)
      .accounts({
    ...
    // Fund reward vaults — each pool has its own token mint as reward
    ...
    // Pool A: reward = tokenMintA
    const adminRewardAccountA = await getOrCreateAssociatedTokenAccount(
      provider.connection,
      admin.payer,
      tokenMintA,
      admin.publicKey
    ...
    await program.methods
      ...
      .accounts({
        pool: poolA,
        admin: admin.publicKey,
        adminRewardAccount: adminRewardAccountA.address,
    ...
    // Pool B: reward = tokenMintB
    const adminRewardAccountB = await getOrCreateAssociatedTokenAccount(
      provider.connection,
      admin.payer,
      tokenMintB,
      admin.publicKey
    ...
    await program.methods
      ...
      .accounts({
        pool: poolB,
        admin: admin.publicKey,
        adminRewardAccount: adminRewardAccountB.address,
